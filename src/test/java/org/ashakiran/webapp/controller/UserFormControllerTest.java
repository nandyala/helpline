...
public class UserFormControllerTest {
  ...
  @InjectMocks private UserFormController controller;
  ...
  @Test public void testEdit() throws Exception {
    ...
    mockMvc.perform(get("/userform").param("id", "1")).andExpect(status().isOk()).andExpect(model().attributeExists(
        "user")).andExpect(model().attribute("user", hasProperty("firstName", is("Matt")))).andExpect(view().name(
        "userform"));
  ...
  @Test public void testRemove() throws Exception {
    ResultActions update = mockMvc.perform(post("/userform").param("id", user.getId().toString()).param("delete", ""))
  ...
  @Test public void testSave() throws Exception {
    ...
    ResultActions update = mockMvc.perform(post("/userform").param("user.lastName", savedUser.getLastName())).andExpect(
...
} // end class UserFormControllerTest
